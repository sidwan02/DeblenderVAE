...
# 'centered/'  # centered/ miscentered_0.1/ miscentered_peak/ #real/
case = str(sys.argv[1])
training_or_test = str(sys.argv[2])  # 'test' # training test validation
isolated_or_blended = str(sys.argv[3])  # 'blended' # isolated blended
# 'noshift' # 'noshift', 'uniform', 'uniform+betaprime'
method_shift = str(sys.argv[4])
...
cosmos_cat = galsim.COSMOSCatalog(
    'real_galaxy_catalog_25.2.fits', dir=cur_dir + '/COSMOS_25.2_training_sample')
# cosmos_cat = galsim.COSMOSCatalog(
#     'real_galaxy_catalog_25.2.fits', dir=cur_dir + '/COSMOS_25.2_training_sample')
cosmos_cat_dir = cur_dir + '/COSMOS_25.2_training_sample'
...
keys = ['nb_blended_gal', 'SNR', 'SNR_peak', 'redshift', 'moment_sigma', 'e1', 'e2', 'mag', 'mag_ir', 'closest_x', 'closest_y', 'closest_redshift', 'closest_moment_sigma', 'closest_e1',
        'closest_e2', 'closest_mag', 'closest_mag_ir', 'blendedness_total_lsst', 'blendedness_closest_lsst', 'blendedness_aperture_lsst', 'idx_closest_to_peak', 'n_peak_detected']
...
for icat in trange(N_files):
    ...
    for z in range(N_per_file):
        res.append(image_generator(cosmos_cat_dir, training_or_test, isolated_or_blended, save_dir,
                   used_idx, nmax_blend, 100, 27.5, method_shift, do_peak_detection, do_add_shear))
    ...
    if training_or_test == 'test':
        ...
        np.save(os.path.join(save_dir, root_i+'_I_norm.npy'),
                np.array(np.mean(max_i_noisy, axis=1)))
